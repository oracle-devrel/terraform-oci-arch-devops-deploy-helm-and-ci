...
resource "oci_identity_dynamic_group" "devopsgroup1" {
  ...
  name           = "devopsdyngroup-${var.app_name}-${random_id.tag.hex}"
...
resource "oci_identity_dynamic_group" "devopsgroup2" {
  provider       = oci.home_region
  name           = "CodeReposDynamicGroup-${var.app_name}-${random_id.tag.hex}"
...
resource "oci_identity_dynamic_group" "devopsgroup3" {
  provider       = oci.home_region
  name           = "MyDynamicGroup-${var.app_name}-${random_id.tag.hex}"
